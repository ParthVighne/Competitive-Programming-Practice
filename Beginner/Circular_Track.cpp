...
int main()
{
    ...
    for (int i = 0; i < T; i++)
    {
        ...
        /*
            ...
            1 -> positive distance between A & B is B-A;
            2 -> if B-A is shorter than it's reverse path i.e. M - ( B - A ), then choose the prior
            3 -> else choose the latter.
        */
        int F = abs(B - A); // abs() returns positive value
        int R = abs(M - F);
        if (F <= R)
            cout << F << endl;
        else
